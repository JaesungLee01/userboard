...
<%
	...
	if(request.getParameter("localName") == null				// 지역명이 null이거나 공백이면
			...
			response.sendRedirect(request.getContextPath()+"/board/updateCategoryForm.jsp?localName="+oldLocalName);// 카테고리 수정 폼으로
			return;
	}
	// 요청값 변수에 저장
	String localName = request.getParameter("localName");
	// 디버깅
	System.out.println(localName + " <-- updateCategoryAction localName");
	...
	// 위 sql 디버깅
	System.out.println(updateLocalStmt + " <-- updateCategoryAction localStmt");
	// 전송한 sql 실행값 반환
	// db쿼리 결과셋 모델
	...
	if(row == 1){ // 카테고리 수정 성공
		...
		response.sendRedirect(request.getContextPath()+"/board/updateCategoryForm.jsp?localName="+oldLocalName);
		return;
	}
